...
FILE * trace;
PIN_LOCK pinLock;
static unsigned long long machine_access = 0;

void countMemoryAccess(int memSize, int tid, unsigned long long memoryAddress){
...
VOID RecordMachineAccess(VOID * ip, THREADID tid, int memSize, void * addr)
{    
    ...
    if (block_end >= memSize){
        countMemoryAccess(memSize,tid,memoryAddress);
    }
    else{
        countMemoryAccess(block_end,tid,memoryAddress);
        countMemoryAccess(memSize - block_end,tid,memoryAddress + block_end);
    }
...
VOID Fini(INT32 code, VOID *v)
{
    ...
    fprintf(trace, "#eof\n");
    fclose(trace);
    fprintf(stdout, "Output trace file generated - input/thread_memory_trace.txt\n");
}
...
//	sprintf(inputname, "input/thread_memory_trace_%s.txt",argv[1]);
	trace = fopen("input/thread_memory_trace.txt", "w");
